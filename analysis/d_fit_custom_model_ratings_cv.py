...
def pred_ratings(parameters, data):
    """
    Makes predictions for aesthetic ratings on a trial-by-trial basis
    given model parameters and a data frame with trial info

    Parameters
    ----------
    parameters : list or 1D array
        parameter values that are not fixed and need to be passed to the model.
    data : pd DataFrame
        Data frame that contains trial-by-trial information about the
        features representing each stimulus and viewing time where each
        row of the data frame is one trial.

    Returns
    -------
    pred : 1D array
        Array of predicted ratings in the order of the trials as they
        appear in data.
...
def cost_fn(parameters, data, testPair, test=False):
    ...
    data : pd DataFrame
        ...
        row of the data frame is one trial.
    testPair : str
        identifier string for the test pair.
    test : bool, optional
        Indicates whether RMSE calculations are based on all but the test pair
        (if set to False) or only the test pair. The default is False.
...
def L2_const(parameters):
    ...
    -------
    int
        constraint value to be passed into the constraint dictionary for the
